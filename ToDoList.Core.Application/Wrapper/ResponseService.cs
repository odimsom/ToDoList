...
namespace ToDoList.Core.Application.Wrapper
{
    public class ResponseService<TData> : OperationResult<TData>
    {
        ...
        public OperationResult<TData> OperationResult { get; set; } = new();
        ...
        public ResponseService(TData data, string message)
        {
            StatusCode = 200;
            OperationResult = OperationResult<TData>.Success(data, message);
        }

        // Constructor for simple failure responses
        public ResponseService(string errorMessage)
        {
            StatusCode = 400;
            OperationResult = OperationResult<TData>.Failure(new List<string> { errorMessage });
        }
